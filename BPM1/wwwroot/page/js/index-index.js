...
//让工作条目下边的线条滑动
	...
	function BarAnimate(that){
		...
		var $padwidth=$('.dot-container').width()*0.085
		...
		that.find('.item').addClass('current').parents('.item-container').siblings().find('.item').removeClass('current')		
		var $wid=$containerwidth*$index
		var $dotlinwidth=$('.dot-line').width()
		var $barbox=$('.bar-box')
		$barbox.css({
			'padding-left':$padwidth-7.5+'px'
		})
		//点击线的长度变化
